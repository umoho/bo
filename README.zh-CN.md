...
**bo** 是一个命令驱动的音频剪辑、混音与播放工具。你用 `put` 命令描述一个会话——把片段放到叠加的音轨上，每个片段是音频源的一个切片——用 `set` 调混音，再用 `play`（走声卡）或 `render`（离线混成 wav）听结果。没有项目文件：会话只活在常驻 daemon 的内存里，通过 Unix socket 访问，可以写成命令脚本（`save`），也可以从脚本重建（`load`）。
