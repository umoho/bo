...
- **曲线即手势** —— `set clip.N.N.curve 0:1,3.2:-1` 把一条曲线插进 clip 的声像，`set clip.N.N.gain_curve` 插进它的音量：听到的是静态基准加曲线随 clip 播放而变，现场播放与离线渲染完全一致（两边构建同一条链），改动像 fade 一样即时落在正在跑的混音上——不需要脚本去轮询播放头。曲线是第一个**控制源**；将来的 LFO、sidechain 的包络跟随器是更多的源，曲线能骑的输入孔是 clip 的声像与音量。
...
演示里的"右扫左"曾经要一个后台脚本轮询播放头、每几十毫秒重摆一次位。变成曲线后只是一行，播放与渲染同源：

```console
$ bo put slide.wav,00:00:00-00:00:03.200
ok: 1 clip on track 0
$ bo set clip.0.0.curve 0:1,3.2:-1    # 声像随 clip 从 +1 走到 -1
ok: `clip.0.0.curve` set to `0:1,3.2:-1`
$ bo render mix.wav                   # 渲染出的文件和播放听到的是同一条扫
```

曲线是 `时刻:值` 关键点（从 clip 起点算秒，点间线性、点外 hold），叠加在参数的静态基准上——clip 的声像（`set clip.N.N.curve`）或音量（`set clip.N.N.gain_curve`）；`none` 拔线。参数 = 静态基准 + Σ(活跃控制源)；每个输入孔今天各接一条曲线。
