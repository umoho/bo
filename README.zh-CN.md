...
> 给 agent 用的音频剪辑与混音工具。一次一条命令——搭音轨、摆切片、调混音，然后实时试听或离线渲染成文件。现在还不是 DAW，但已经长着 DAW 的骨架。

[English](README.md) | [中文](README.zh-CN.md)

**bo** 是一个命令驱动的音频剪辑与混音工具。你用 `put` 命令描述一个会话——把片段放到叠加的音轨上，每个片段是音频源的一个切片——用 `set` 调混音，再用 `play`（走声卡）或 `render`（离线混成 wav）听结果。没有项目文件：会话只活在常驻 daemon 的内存里，通过 Unix socket 访问，可以写成命令脚本（`save`），也可以从脚本重建（`load`）。

每次 `bo ...` 调用就是一个动作：放一个片段、移除一个、移动播放头、压低某条音轨、开始或停止播放。daemon 按需自动拉起、用后自动清理，所以一次会话是一场对话，而不是一个项目。

## bo 是什么——以及不是什么

**bo** 是给 agent 用的剪辑与混音工具：切片源、把片段摆上时间轴、叠音轨、调音量与静音、渲染或试听。
...
- **实时试听，或离线渲染** —— 通过声卡试听（rodio），或把整个编排——乃至其中一段区间——离线混音成 wav。
