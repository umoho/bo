...
- **源即手势** —— `set clip.N.N.pan_control 0:1,3.2:-1` 把一条曲线插进 clip 的声像，`set clip.N.N.gain_control sine:2:0.3:0` 把一个 LFO 插进它的音量：听到的是静态基准加源随 clip 播放而变，现场播放与离线渲染完全一致（两边构建同一条链），改动像 fade 一样即时落在正在跑的混音上——不需要脚本去轮询播放头。曲线与 LFO 是最初两个**控制源**；sidechain 的包络跟随器是后来的一个，源能骑的输入孔是 clip 的声像与音量。
