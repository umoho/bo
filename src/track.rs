...
//! ([`crate::bus`]) and the control layer ([`crate::control`]): nothing here
//! says how a sound is *mixed* or *modulated* — the clips merely carry the
//! control sources ([`ControlSource`]) their parameters can be driven by. No
//! transport, no decoding, no metadata — just the shape those layers sit on.
