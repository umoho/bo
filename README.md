...
- **A curve is a gesture** — `set clip.N.N.curve 0:1,3.2:-1` plugs a curve into a clip's pan: the pan rides the static placement plus the curve as the clip plays, identically live and rendered (both build the same chain), and an edit lands on the running mix like a fade. No script polling the playhead. The curve is the first *control source*; an LFO and a sidechain's envelope follower are later ones, and further parameters (gain) get inputs of their own.
...
between them, held flat at the edges) offsetting the clip's static pan;
`none` unplugs it. The parameter is the static base plus the sum of its
sources — today one curve on one input, pan, with a slot for more.
