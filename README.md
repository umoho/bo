...
- **A curve is a gesture** — `set clip.N.N.curve 0:1,3.2:-1` plugs a curve into a clip's pan, `set clip.N.N.gain_curve` into its gain: the parameter rides the static base plus the curve as the clip plays, identically live and rendered (both build the same chain), and an edit lands on the running mix like a fade. No script polling the playhead. The curve is the first *control source*; an LFO and a sidechain's envelope follower are later ones, and the inputs a curve can ride are the pan and the gain of a clip.
...
### A curve instead of a script

The demo gesture once needed a background script that polled the playhead and
re-panned every few tens of milliseconds. As a curve it is one line, live and
rendered alike:

```console
$ bo put slide.wav,00:00:00-00:00:03.200
ok: 1 clip on track 0
$ bo set clip.0.0.curve 0:1,3.2:-1    # pan rides +1 → -1 over the clip
ok: `clip.0.0.curve` set to `0:1,3.2:-1`
$ bo render mix.wav                   # the file sweeps the same way it plays
```

A curve is `time:value` keyframes (seconds from the clip's start, linear
between them, held flat at the edges) offsetting the parameter's static
base — a clip's pan (`set clip.N.N.curve`) or its gain
(`set clip.N.N.gain_curve`); `none` unplugs it. A parameter is the static
base plus the sum of its sources; each input takes one curve today.
