...
- **A source is a gesture** — `set clip.N.N.pan_control 0:1,3.2:-1` plugs a curve into a clip's pan, `set clip.N.N.gain_control sine:2:0.3:0` an LFO into its gain, and `set clip.N.N.gain_control sidechain:group.0:-1.5:0.005:0.12` a duck under a bus: the parameter rides the static base plus the source as the clip plays, identically live and rendered (both build the same chain), and an edit lands on the running mix like a fade. No script polling the playhead. Three *control sources* today — curve, LFO, sidechain — ride the pan and the gain of a clip.
...
- a *sidechain* — listens to a bus (the master, or a group you routed) and
  follows its level, as `sidechain:BUS:AMOUNT[:ATTACK:RELEASE]`: amount is
  the offset per unit level — negative ducks, positive swells — and the
  times are seconds (defaults: 5 ms attack, 150 ms release). The radio
  gesture is one line: route the voice into a group and put
  `sidechain:group.0:-1.5` on the music's gain — the music rides down under
  the voice and swells back when it ends.
